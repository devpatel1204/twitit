...
const Userpage=() => {
    ...
    const userSaver = async () => {
        try {
            await axios.get(`${url}/user/search`)
            .then ((res) => {
                        ...
                        userdata[0].map((post, index) => (
                            <Col className = "mb-3" style={{display:'flex'}}>
                                <Card  style = {{display:'flex' , backgroundColor: 'white', height:80,width:300, color: '#ffffff', borderWidth: '2px' , borderBlockColor:'orange'}}>
                                    <Card.Body> 
                                        <Card.Text>                                       
                                           <span style={{color:'black', fontWeight:600, display:'flex'}}><h4>    {post.username}</h4> <Button style={{marginLeft:'auto'}} value={post.username} onClick={(e) => followclickdone(e) }> Follow </Button> </span>
